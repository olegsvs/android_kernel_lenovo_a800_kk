...
//gionee wangzhen 2012-09-6 add for gionee sec func begin
#if defined (GN_RM_LK_SUPPORT)
#include <linux/gn_sec.h>
static UINT32 para[2];
#endif
//gionee wangzhen 2012-09-6 add for gionee sec func end
...
//gionee wangzhen 2012-09-6 add for gionee sec func begin
		...
		if(!go_rm_verified()) 
		{
			g_NewSensorExpGain.u2SensorNewExpTime = 0x120;
			g_NewSensorExpGain.u2SensorNewGain = 0x50;
		} 
		else
		{
...
//gionee wangzhen 2012-09-6 add for gionee sec func begin
#if defined (GN_RM_LK_SUPPORT)
            if((pFeatureCtrl->FeatureId == SENSOR_FEATURE_SET_YUV_CMD)&&!go_rm_verified()) {
            		para[0] = FID_AWB_MODE;
        			para[1] = AWB_MODE_FLUORESCENT;
            		g_pSensorFunc->SensorFeatureControl(pFeatureCtrl->FeatureId,(unsigned char*)&para[0],(unsigned int*) &FeatureParaLen);
            } else  {
