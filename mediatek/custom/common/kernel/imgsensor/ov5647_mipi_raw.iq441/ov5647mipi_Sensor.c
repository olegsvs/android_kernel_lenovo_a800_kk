...
kal_uint16 OV5647MIPI_check_otp_wb(kal_uint16 index)
{
	...
	/* Gionee Wujh modify for otp read fail,20120731, start */
  OV5647MIPI_write_cmos_sensor(0x0100, 0x01);	
	mdelay(1);
	/* Gionee Wujh modify for otp read fail,20120731, end */
...
if(iShutter< (OV5647MIPI_sensor.pclk/(OV5647MIPI_sensor.line_length+50)/100))
{
     OV5647MIPISetMaxFrameRate(250);
     extra_line = (OV5647MIPI_sensor.pclk *10 / (OV5647MIPI_sensor.line_length * 250));
...
/* Gionee Qux. Correct the IO settings. 08 27th, 2012 */
#if 1 
	OV5647MIPI_write_cmos_sensor(0x3000, 0x00);
	OV5647MIPI_write_cmos_sensor(0x3001, 0x00);
	OV5647MIPI_write_cmos_sensor(0x3002, 0x00); //Qux
#else
	OV5647MIPI_write_cmos_sensor(0x3000, 0xfc);
	OV5647MIPI_write_cmos_sensor(0x3001, 0x00);
	OV5647MIPI_write_cmos_sensor(0x3002, 0x1f);
#endif
/* Gionee Qux. END */

	OV5647MIPI_write_cmos_sensor(0x3011, 0x02);
	OV5647MIPI_write_cmos_sensor(0x0100, 0x00);
...
//GIONEE suxt 20120621 add for 13A camera mirror begin	 
	...
	OV5647MIPI_write_cmos_sensor(0x3821, 0x00); //suxt modify
	OV5647MIPI_write_cmos_sensor(0x3820, 0x47); //suxt modify
#else
...
#ifdef GN_MTK_BSP_MAIN_CAMMERA_HVMIRROR
	OV5647MIPI_write_cmos_sensor(0x3821, 0x00); //suxt modify
	OV5647MIPI_write_cmos_sensor(0x3820, 0x06); //suxt modify47
#else
	OV5647MIPI_write_cmos_sensor(0x3821,0x06); 
...
UINT32 OV5647MIPIOpen(void)
{
	...
	kal_uint32 module_id; //Gionee Qux
...
//Default level is for Ac module
	...
	SENSORDB("OV5647MIPIOpen__:%x\n", module_id);

	if(module_id == MODULE_ID_TRULY)
		return ERROR_SENSOR_CONNECT_FAIL;

	SENSORDB("OV5647MIPIOpen Sunny initial...\n");
...
//Gionee:Qux
	module_id = OV5647GetModuleID();
	SENSORDB("OV5647MIPIOpen module:%x\n", module_id);
...
UINT32 OV5647GetModuleID(void)
{
	kal_uint32 module_id = 0x0;
	kal_uint32 i = 0;

	OV5647MIPI_write_cmos_sensor(0x0100,0x01); //exit sleep
	mdelay(5);
	...
	while (i++ < 3) {
		...
		module_id = OV5647MIPI_read_cmos_sensor(0x3D05);
		SENSORDB("OV5647MIPIOpen, 3d05rg_va:%x [%d]\n", module_id, i);
		SENSORDB("OV5647MIPIOpen, module_id:%x \n", module_id & 0x7f);
		OV5647MIPI_write_cmos_sensor(0x3D21, 0x00);
	}

	//OV5647MIPI_write_cmos_sensor(0x0100, 0x00); //resume
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	mdelay(40);   //Gionee yanggy 2012-08-10 modify for CR00663749
